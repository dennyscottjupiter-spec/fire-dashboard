...
**Always sanity-check rendered numbers for realism, not just "tests pass."** A test suite can be green while the actual rendered output is absurd (e.g. the Event Simulator's v2.6 bug: every historical crash rendered a slide to ~€0 because the replay window was too short to include the recovery years — tests never caught it because they only checked "diverges from steady," not "is this plausible"). Before calling a projection/chart change done: read the actual live numbers (chart data, KPIs) in Chrome MCP and ask "would a real financial planner believe this curve?" — dips should recover, growth should compound, nothing should silently zero out unless the inputs genuinely justify depletion.
...
- `js/data.js` — **vendored historical dataset**, no DOM: `HIST` (S&P 500 total return + US CPI, 1926–2023), `VINTAGES` (5 infamous crash events, each with a hard-coded 11-year `returns` array — crash year + 10 following years — so the Event Simulator replays the full dip-and-recovery arc, not just the crash).
